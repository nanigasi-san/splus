...
\subsection{グラフの拡張}
グラフを拡張して、\splus の対象となりうる辺がすべて出てくるようにする。
\begin{center}
    \ikeg{extend_default.jpg}
\end{center}
\subsubsection{頂点の分割のアルゴリズム}
頂点内の紐に対してS+を適用したいので、一つの頂点を明示的に分割し、間を繋ぐ2辺を追加する。繋ぐ辺の向きは二種類存在する(軸に対して垂直/平行)
